...
exports.checkBody = [
  ...
  check("difficulty", "Difficulty can only be easy, medium or hard").toLowerCase().notEmpty().isIn(["easy", "medium", "hard"]),
  check("price", "Price must be greater than 0").notEmpty().isFloat({ min: 0 }),
  check("groupSize", "Group size can't be exceeded more than 10").notEmpty().isFloat({
    min: 0,
    max: 10,
  }),
